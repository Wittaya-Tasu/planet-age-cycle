...
assert.match(indexSource, /v0\.9\.3/);
assert.match(indexSource, /id="target-date-day"/);
assert.match(indexSource, /id="target-date-month"/);
assert.match(indexSource, /id="target-date-year"/);
assert.doesNotMatch(indexSource, /id="target-date" type="date"/);
...
assert.match(layoutCss, /grid-auto-flow: column/);
...
assert.match(swSource, /css\/annual\.css/);
assert.match(swSource, /js\/core\/annualForecast\.js/);
assert.match(swSource, /js\/components\/annual-view\.js/);
assert.match(swSource, /data\/annual-forecast\.json/);
const annualSource = await readFile(`${root}js/core/annualForecast.js`, "utf8");
const annualViewSource = await readFile(`${root}js/components/annual-view.js`, "utf8");
const annualCss = await readFile(`${root}css/annual.css`, "utf8");
assert.match(indexSource, /id="page-tab-annual"/);
assert.match(indexSource, /id="annual-page"/);
assert.match(indexSource, /id="annual-age-basis"/);
assert.match(appSource, /buildAnnualForecast/);
assert.match(appSource, /renderAnnualForecast/);
assert.match(annualSource, /gregorian_proportional_projection|projectionMode/);
assert.match(annualSource, /calculateThaloengSokBoundary/);
assert.match(annualViewSource, /annual-relation-connectors/);
assert.doesNotMatch(annualViewSource, /statusBadge\(/);
assert.doesNotMatch(annualViewSource, /annual-piece-mark/);
assert.match(annualCss, /annual-forecast-svg/);

console.log("✓ มหาทศา v0.9.3: ลำดับ 108 ปีและดาวแทรกยังถูกต้อง");
...
console.log("✓ PWA cache เป็น maha-thasa-v0.9.3");
